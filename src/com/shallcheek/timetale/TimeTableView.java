...
public class TimeTableView extends LinearLayout {
    ...
    private final static int TimeTableTextSize = 13;
    ...
    int colornum = 0;
    //数据源
    private List<Timetable> mListTimeTable = new ArrayList<Timetable>();
        ...
        for (int i = 0; i <= WEEKNUM; i++) {
            switch (i) {
                ...
                case 7:
                    ...
                    List<Timetable> mListMon = new ArrayList<Timetable>();
                    //遍历出星期1~7的课表
                    for (Timetable timeTable : mListTimeTable) {
                        if (timeTable.day == i) {
                            mListMon.add(timeTable);
                        }
                    }
                    //添加
     ...
     * @return
     */
    private LinearLayout getTimeTableView(List<Timetable> model, int week) {
        ...
        if (modesize <= 0) {
            ...
            for (int i = 0; i < modesize; i++) {
                if (i == 0) {
                    //添加的0到开始节数的空格
                    mTimeTableView.addView(addStartView(model.get(0).startclass, week, 0));
                    mTimeTableView.addView(getMode(model.get(0)));
                } else if (model.get(i).startclass - model.get(i - 1).startclass > 0) {
                    //填充
                    mTimeTableView.addView(addStartView(model.get(i).startclass - model.get(i - 1).endclass, week, model.get(i - 1).startclass));
                    mTimeTableView.addView(getMode(model.get(i)));
                }
                if (i + 1 == modesize) {
                    mTimeTableView.addView(addStartView(MAXNUM - model.get(i).endclass, week, model.get(i).endclass));
                }
            }
        }
    ...
    private View getMode(final Timetable model) {
        LinearLayout mTimeTableView = new LinearLayout(getContext());
        mTimeTableView.setOrientation(VERTICAL);
        TextView mTimeTableNameView = new TextView(getContext());
        int num = model.endclass - model.startclass;
        ...
        mTimeTableNameView.setTextSize(TimeTableTextSize);
        mTimeTableNameView.setPadding(dip2px(2), 0, dip2px(2), 0);
        mTimeTableNameView.setGravity(Gravity.CENTER);
        mTimeTableNameView.setText(model.name + "@" + model.address);
        ...
        mTimeTableView.setBackgroundDrawable(getContext().getResources()
                .getDrawable(colors[getColorNum(model.name)]));
        mTimeTableView.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                Toast.makeText(getContext(), model.name + "@" + model.address, Toast.LENGTH_LONG).show();
            }
        });
        return mTimeTableView;
    }
    ...
    public void setTimeTable(List<Timetable> mlist) {
        this.mListTimeTable = mlist;
        for (Timetable timeTable : mlist) {
            addTimeName(timeTable.name);
        }
        initView();
        invalidate();
    }
